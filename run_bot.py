...
class OnRebalanceResult(NamedTuple):
  ...
  tick_drift_cooldown: bool = False
...
async def main() -> None:
  ...
  if not getattr(cfg.portfolio, "auto_rebalance_when_stopped", False):
    logger.warning(
      ...
      "Включите portfolio.auto_rebalance_when_stopped: true в config.yaml или нажмите Старт."
    )
  ...
  async def on_rebalance(source: str = "manual") -> OnRebalanceResult:
    """source: 'manual' | 'schedule' | 'drift' — для логов и планировщика."""

    def _out(msg: str, sched: bool, drift: bool) -> OnRebalanceResult:
      if source == "manual":
  ...
  async def on_rebalance_for_tg() -> str:
  ...
  tg.set_callbacks(
    ...
    on_status=on_status,
    on_rebalance=on_rebalance_for_tg,
  ...
  async def daily_digest_scheduler():
    ...
    try:
      digest_h, digest_m = map(int, str(digest_time).strip().split(":"))
    except (ValueError, AttributeError):
      digest_h, digest_m = 18, 0
    digest_slot_mins = digest_h * 60 + digest_m
    tz_label = (getattr(cfg.portfolio, "trading_timezone", None) or "").strip() or "локально"
    last_digest_date: date | None = None
    last_alert_drawdown_date: date | None = None
    last_alert_daily_loss_date: date | None = None
    logger.info(
      "daily_digest_scheduler: запущен, время=%s (минута дня=%d), TZ=%s",
      digest_time,
      digest_slot_mins,
      tz_label,
    )
    ...
    while True:
      ...
      try:
        ...
        if last_digest_date == today or now_slot < digest_slot_mins:
          continue
        ...
        if ok:
          ...
          logger.warning(
            "Дневной дайджест: не доставлен в Telegram, дата не отмечена — повтор на следующей минуте",
          )
        ...
        if (
          alert_daily > 0
          and st.daily_pnl < 0
          and (-st.daily_pnl / day_start_val) >= alert_daily
          and last_alert_daily_loss_date != today
        ):
          last_alert_daily_loss_date = today
          await send_alert(
            tg,
            f"⚠️ Дневной убыток {st.daily_pnl:.2f} ({-st.daily_pnl / day_start_val:.1%}) превысил порог {alert_daily:.1%}.",
            "alert_daily_loss",
            force=True,
          )
      except Exception:
        logger.exception("daily_digest_scheduler: ошибка в тике")
  ...
  async def auto_rebalance_scheduler():
    ...
    try:
      wp = [p for p in str(weekly_time).strip().split(":") if p.strip() != ""]
      wh, wm = int(wp[0]), int(wp[1])
    except (ValueError, AttributeError, IndexError):
      wh, wm = 18, 0
    ...
    raw_ri_h = getattr(cfg.portfolio, "rebalance_interval_hours", 0.0) or 0.0
    try:
      ri_hours = max(0.0, float(raw_ri_h))
    except (TypeError, ValueError):
      logger.warning("Планировщик: некорректное rebalance_interval_hours=%r, используем 0 (раз в день)", raw_ri_h)
      ri_hours = 0.0
    ...
    try:
      ...
      logger.exception("Планировщик: rebalance_day_minutes_window недоступно, используем 10:00–23:59")
      window_start_mins, window_end_mins = hour * 60 + minute, 24 * 60 - 1
    ...
    logger.info(
      "Планировщик ребаланса: время=%s, окно=%s–%s (%s), drift=%s порог=%.2f интервал=%dм cooldown=%dм, по расписанию каждые %.2f ч (0 = раз в день)",
      rebalance_time,
      ...
      try:
        ...
        if scheduler_ok:
          ...
          interval_sched_due = ri_hours > 0 and (
            last_interval_rebalance_at is None
  ...
  logger.info("Планировщик: задачи созданы (scheduler_task, daily_digest_scheduler)")
